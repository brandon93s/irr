...
const convert_rate_1 = require("./convert-rate");
const EPSILON = 1e-10;
const TEST_CASES = [
    {
        input: 0.002,
...
describe('convertRate', () => {
    TEST_CASES.forEach(testCase => {
        test(`${testCase.from} -> ${testCase.to}`, () => {
            const result = (0, convert_rate_1.convertRate)(testCase.input, testCase.to, testCase.from);
            expect(Math.abs(result - testCase.output)).toBeLessThan(EPSILON);
        });
    });
    test('uses default = day', () => {
        const result = (0, convert_rate_1.convertRate)(0.001, convert_rate_1.RateInterval.Month);
        expect(Math.abs(result - 0.030439087548097543)).toBeLessThan(EPSILON);
    });
});
//# sourceMappingURL=convert-rate.test.js.map